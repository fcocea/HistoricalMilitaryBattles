...
def create_waffle_chart(data, height, width, categories, colorscale, selected_country, y_offset):
    ...
    category_proportions = {category: float(
        value) / total_values for category, value in zip(data.index, data['Total'])}
...
def update_battle_troops(selected_country, selected_contrincante=None):
    if selected_contrincante is None:
        ...
        troops['country'] = np.where(
            troops['attacker_y'] == 1, troops['attacker_x'], troops['defender'])
        troops.drop(['attacker_x', 'defender', 'attacker_y'],
                    axis=1, inplace=True)

        troops = troops[troops['country'] == selected_country]
        troops = troops[['country', 'tank', 'arty', 'fly', 'cav']]
        troops.columns = ['country', 'tanks',
                          'artillery', 'aircraft', 'cavalry']
